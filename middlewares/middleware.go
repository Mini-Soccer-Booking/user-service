...
func validateAPIKey(c *gin.Context) error {
	...
	request := c.GetHeader(constants.XRequestAt)
	serviceName := c.GetHeader(constants.XServiceName)
	signatureKey := config.Config.SignatureKey

	validateKey := fmt.Sprintf("%s_%s_%s", serviceName, request, signatureKey)
...
func validateBearerToken(c *gin.Context, token string) error {
	...
	tokenString := extractBearer(token)
	if tokenString == "" {
		return errConstant.ErrUnauthorized
	}
...
func Authenticated() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if err != nil {
			responseUnauthorized(c, "JWT Error: "+err.Error())
			return
		}

		err = validateAPIKey(c)
		if err != nil {
			responseUnauthorized(c, "API Key Error: "+err.Error())
			return
		}
